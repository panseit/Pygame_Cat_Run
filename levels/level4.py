...
def init_variables():
	...
	data.LEVEL_4_EXIT = data.game_paused = False
	data.CURRENT_PAUSE_SELECTION = 0
	
	global cat_hero
	data.score = 0
        data.score_surface = data.score_font.render("Score:" + str(data.score)+"/1750", 1, data.WHITE)
	
	cat_hero = object_animations.Cat_Hero(data.WINDOWWIDTH/100, data.WINDOWHEIGHT-100)
	
	global level_4_file
	try:
		level_4_file = open(path.join("resources", "levels", "level4.dat"))
	except IOError:
		print "The file", path.join("resources", "levels", "level4.dat"), "does not exist! The game will now exit!"
		data.terminate(1)
	else:
		#read the first WINDOWWIDTH/20 lines and blit them to the screen...
		for i in range(65):
			process_level_line(level_4_file.readline(), i*10)
			data.floors.add(object_animations.Floor(i*10, data.level4_floor_image))
		data.backgrounds.add(object_animations.Background(0, data.level4_bg_image))
		data.backgrounds.add(object_animations.Background(data.backgrounds.sprites()[0].width, data.level4_bg_image))
...
def process_level_line(line, xpos):
	...
	for char in line:
		if(char in data.barriers_syms):
			if char == "@":
				data.barriers.add(object_animations.Barrier(xpos, data.WINDOWHEIGHT-counter*20, data.level4_christmas_goblin_image))
			elif char == "#":
				data.barriers.add(object_animations.Barrier(xpos, data.WINDOWHEIGHT-counter*20, data.level4_snowman_image))
			elif char == "$":
				data.barriers.add(object_animations.Barrier(xpos, data.WINDOWHEIGHT-counter*20, data.level4_christmas_sleigh_image))
		elif(char in data.points_syms):
			if char == "%":
				data.points.add(object_animations.Point(xpos, data.WINDOWHEIGHT-counter*20, data.point1l4_image))
			elif char == "^":
				data.points.add(object_animations.Point(xpos, data.WINDOWHEIGHT-counter*20, data.point2l4_image))
		elif(char in data.finish_sym):
			data.finish_line.add(object_animations.Finish_Line(xpos, data.WINDOWHEIGHT-counter*20, data.level4_finish_line_image))
		counter+=1
...
def read_level_file(speed):
	...
	if(data.read_new_line_counter <= 0):
		global level_4_file
		process_level_line(level_4_file.readline(), data.WINDOWWIDTH+data.read_new_line_counter)
...
def check_events():
	...
	if(game.sprite.spritecollide(cat_hero, data.points, True)):
		...
		data.win_sound.play()
		data.level_now = 4
		data.LEVEL_4_EXIT = True
		menu_libs.level_selection.show()
	
def wait_for_level_end():
	while not data.LEVEL_4_EXIT:
		if data.game_paused:
