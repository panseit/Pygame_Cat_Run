...
def process_level_line(line, xpos):
	...
	for char in line:
		if(char in data.barriers_syms):
			if char == "@":
				data.barriers.add(object_animations.Barrier(xpos, data.WINDOWHEIGHT-counter*20, data.level2_rock_image))
			elif char == "#":
				data.barriers.add(object_animations.Barrier(xpos, data.WINDOWHEIGHT-counter*20, data.level2_rat_image))
			elif char == "$":
				data.barriers.add(object_animations.Barrier(xpos, data.WINDOWHEIGHT-counter*20, data.level2_sandcastle_image))
		elif(char in data.points_syms):
			if char == "%":
				data.points.add(object_animations.Point(xpos, data.WINDOWHEIGHT-counter*20, data.point1l2_image))
			elif char == "^":
			...
			data.finish_line.add(object_animations.Finish_Line(xpos, data.WINDOWHEIGHT-counter*20, data.level2_finish_line_image))
		counter+=1
...
def read_level_file(speed):
	...
	if(data.read_new_line_counter <= 0):
		global level_2_file
		process_level_line(level_2_file.readline(), data.WINDOWWIDTH+data.read_new_line_counter)
...
def check_events():
	...
	if(game.sprite.spritecollide(cat_hero, data.points, True)):
		...
		data.win_sound.play()
		data.level_now = 3
		data.LEVEL_2_EXIT = True
		menu_libs.level_selection.show()
	
def wait_for_level_end():
	while not data.LEVEL_2_EXIT:
		if data.game_paused:
